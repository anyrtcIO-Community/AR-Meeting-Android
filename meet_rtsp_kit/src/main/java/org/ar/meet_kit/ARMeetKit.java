            ...
            public void run() {
                ...
                if(!enable) {
                    if (mVideoCapturer != null) {
                        ...
                        nativeSetVideoCapturer(null, mLocalRender);
                        mVideoCapturer = null;
                    }
                } else {
                    if (mVideoCapturer == null) {
                        ...
                        nativeSetVideoCapturer(mVideoCapturer, mLocalRender);
                        nativeSetLocalVideoRender(mLocalRender);
                    } else {
                        nativeSetVideoCapturer(mVideoCapturer, mLocalRender);
                        nativeSetLocalVideoRender(mLocalRender);
                    }
...
//                nativeSetAuidoModel(false, true);
     ...
     * @param lRender
     */
    public void setRtspRender(final long lRender) {
        mExecutor.execute(new Runnable() {
            @Override
            public void run() {
                nativeSetRtspRender(lRender);
            }
        });
    }
     ...
     */
    public int startRecorder(final boolean isNeedVideo, final boolean onlyLocalVideo, final String filePath) {
        if(isNeedVideo) {
            //视频文件后缀名必须是mp4
