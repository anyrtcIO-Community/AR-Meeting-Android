...
//        });
     ...
     */
    protected int startRecorder(final boolean isNeedVideo, final boolean onlyLocalVideo, final String filePath) {
        if(isNeedVideo) {
            //视频文件后缀名必须是mp4
     ...
     * @return 返回值：0：开始录制成功，-1：文件格式不匹配
     */
    public int startConfigRecorder(final boolean isNeedVideo, final String filePath, final int bitrate) {
        if(isNeedVideo) {
            //视频文件后缀名必须是mp4
            if(filePath.lastIndexOf("mp4") == -1) {
                return -1;
        ...
        mExecutor.execute(new Runnable() {
            @Override
            public void run() {
                int ret = nativeStartConfigRecorder(isNeedVideo, filePath, bitrate);
                LooperExecutor.exchange(result, ret);
            }
        });
        return LooperExecutor.exchange(result, 0);
     ...
     * @return 返回值：0：开始录制成功，-1：文件格式不匹配
     */
    public int startRecorder(final boolean isNeedVideo, final String filePath) {
        if(isNeedVideo) {
            //视频文件后缀名必须是mp4
            if(filePath.lastIndexOf("mp4") == -1) {
                return -1;
...
//    private native void nativeSetLocalPeerAVEnable(String publishId, boolean bIsVideo, boolean videoEnable);
    ...
    private native int nativeStartConfigRecorder(boolean isNeedVideo, String filePath, int bitrate);
